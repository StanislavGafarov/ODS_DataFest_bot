...
# Get news
TEXT_NO_NEWS = 'Пока, опубликованных новостей от админов нет ¯\_(ツ)_/¯'
TEXT_NEWS = "Тут можно подписаться на последние обновления, а также посмотреть последние новости о мероприятиях ODS."
...
Админов: {}
'''
TEXT_NEWS_FAIL_LOAD = "Ошибка при получении новостей"
...
TEXT_CONGRATULATION = 'Поздравляю Вы выграли приз 🎉🎉🎉, забрать приз вы сможете вечером на церемонии закрытия на Main Stage'
...
TEXT_RANDOM_BEER_MENU = 'Ваши данные:\n - Telegram nick: {};\n - ODS nick: {};\n - Social network link: {}. Вы можете нажать "Найти партнера", или получить партнера автоматически.'
...
TEXT_END_MEETING = 'Хорошо, мы предложим вам нового партнера автоматически или вы можете сами инициировать встречу нажав «Найти партнера»'
TEXT_NOT_ENOUGH_INFO = 'У Вас заполнено слишком мало полей для участия, заполните хотя бы одно поле.'
TEXT_SHOULD_END_MEETING = 'Прежде чем начать следующую встречу, Вам следует нажать «Я освободился»'
...
TEXT_JETSON_WIN = '''Поздравляем, вы выиграли NVIDIA Jetson Nano Developer Kit!\nНадеемся, с его помощью вы создадите нечто крутое😊 Если вдруг не знаете, с чего начать – к вашим услугам JetBot https://github.com/NVIDIA-AI-IOT/jetbot, open-source DIY проект, который поможет вам создать своего первого робота.\n\nЧтобы получить свой приз, подойдите пожалуйста в 14.30, сразу после начала обеденного перерыва на стойку регистрации.'''
TEXT_JETSON_NOT_SUCCEED ='''Розыгрыш NVIDIA Jetson Nano Developer Kit уже прошел, и, к сожалению, в этот раз вы не выиграли. Но мы верим, что вам обязательно повезет на наших будущих мероприятиях. А пока что предлагаем вам посмотреть каталог вебинаров NVIDIA (https://www.nvidia.com/en-us/about-nvidia/webinar-portal/) , наверняка там найдется что-то интересное вам. А если вы работаете в стартапе в области AI, вступайте в нашу программу Inception www.nvidia.ru/Inception - мы предоставляем доступ к техническим, бизнес-и маркетинговым ресурсам.\n\nБольшое спасибо за участие, остаемся на связи.'''
...
TEXT_SHOW_SCHEDULE = "Актуальную информацию по программе мероприятия можно найти тут: https://datafest.ru/elka/"
...
TEXT_YOU_BANNED = 'Ты видимо сделал, что-то очень плохое 👹 за это мы забанили часть функций бота. Больше так не делай!'

############# BUTTONS #############
# Main menu
BUTTON_CHECK_REGISTRATION = 'Проверить статус регистрации'
BUTTON_AUTHORISATION = 'Авторизоваться'
BUTTON_SCHEDULE = 'Программа Data Ёлки 2019 📝'
BUTTON_NEWS = 'Новости 📰'
...
BUTTON_END_MEETING = 'Я освободился, и готов к новой встрече'
